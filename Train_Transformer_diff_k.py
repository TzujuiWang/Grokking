...
def train_model(model, optimizer, criterion, train_data, train_labels, test_data, test_labels,
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for batch_inputs, batch_targets in test_loader:
                ...
                correct_test += (predicted_test == batch_targets).sum().item()
...
if __name__ == "__main__":
    ...
    for K in [3, 4]:
        for alpha in [0.3, 0.5, 0.7]:
            save_path = f"alpha_{alpha}_k_{K}_accuracy_curves_{model_type}.png"
            print(f"Training with alpha={alpha}, k={K}...")
            ...
            model = select_model(
                model_type,
                input_dim=p + 2,  # 确保 input_dim 为 p + 2
            ...
            # 绘制结果
            plot_metrics(train_losses, test_losses, "Loss", f"{model_type} Loss for alpha={alpha}, K={K}")
            plot_accuracy(train_accuracies, test_accuracies, f"{model_type} Accuracy for alpha={alpha}, K={K}", save_path)
